const store = {
  ...
  mutations : {
    setData(state, {addresses : addresses}) { state.addresses = addresses; },
    updateEmail(state, email) { state.email = email; },
    updateBuilding(state, building) { state.building = building; },
    updateFloor(state, floor) { state.floor = floor; },
    updateStreet(state, street) { state.street = street; },
    updateAreaName(state, areaName) { state.area_name = areaName; },
    updateApartment(state, apartment) { state.apartment = apartment; },
    updateMessage(state, value) {
      state.message = value;
      return value;
    },
    nextPage(state) { state.pagination++; },
    prePage(state) { state.pagination--; },
    udateMail(state, {user : user}) { state.user = user; },
    updateSearchArea(state, {areas : areas}) { state.searchArea = areas; }
  },
  actions : {
    updateData({commit, state}) {
      ...
      Nova.request()
          ...
          .then(data => {
            ...
            commit({type : "setData", addresses : result});
          })
    ...
    submitData({commit, state}) {
      ...
      Nova.request()
          ...
          .then(data => {
            console.log("Message",
                        commit({type : "updateMessage", value : data.status}));
          })
          .catch(error => {
            console.error(
                "Error:",
                commit(
                    {type : "updateMessage", value : error.response.status}));
          });
    },
    getMail({commit, state}) {
      Nova.request()
          ...
          .then(data => {
            ...
            commit({type : "udateMail", user : field});
          })
          .catch(err => console.log(err.message));
    },
    searchArea({commit, state}, text) {
      Nova.request()
          ...
          .then(data => {
            ...
            commit({type : "updateSearchArea", areas : areas});
          })
          .catch(err => console.log(err.message));
    }
  }
